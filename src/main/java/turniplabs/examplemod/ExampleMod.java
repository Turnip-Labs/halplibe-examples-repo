...
public class ExampleMod implements ModInitializer, GameStartEntrypoint {
	...
	public void beforeGameStart() {
		...
		directionCube = standardBlockBuilder.build("direction", startingBlockId++, b -> new BlockLogic(b, Material.stone));
		...
		stoneSide = standardBlockBuilder
			...
			.build("stoneSide", startingBlockId++, b -> new BlockLogic(b, Material.grass));
		...
		customBlockModel = new BlockBuilder(MOD_ID)
			...
			.build("customModel", startingBlockId++, BlockLogicTorch::new);
		...
		customBlockModel2 = new BlockBuilder(MOD_ID)
			...
			.build("customModel2", startingBlockId++, b -> new BlockLogicCustomShape(b, Material.sponge)) // Set out custom block that goes with our custom model
